...
function usefulData(obj) {
    ...
    forecast.humidity = obj.main.humidity;
    forecast.wind = mpsToMph(obj.wind.speed);
...
function mpsToMph(speed) {
    let newSpeed = (Math.round(speed * 22.3694)) / 10;
    return newSpeed;
}
...
function displayWeather(data) {
    console.log(data);
    dayOrNight(data.timeZone);
    displayIcon(data.description);
    document.getElementById('description').innerText = capitalise(data.description);
    ...
    if (timeOfDay === 'night') {
        ...
        document.getElementsByTagName(svg).style.color = 'white';
    } else {
        document.body.style.backgroundImage = 'url("./clearsky.jpg")';
        document.body.style.color = 'black';
        searchBtn.style.color = 'black';
        searchBtn.style.border = 'solid 1px black';
        document.getElementsByTagName(svg).style.color = 'black';
    }
}

function displayIcon(description) {
    if (description === 'clear sky' && timeOfDay === 'day') {
        icon.innerHTML = clearDay;
    } else if (description === 'clear sky' && timeOfDay === 'night') {
        icon.innerHTML = clearNight;
    } else if (description === 'few clouds' && timeOfDay === 'day') {
        icon.innerHTML = partlyCloudyDay;
    } else if (description === 'few clouds' && timeOfDay === 'night') {
        icon.innerHTML = partlyCloudyNight;
    } else if (description === 'scattered clouds' || description === 'broken clouds') {
        icon.innerHTML = brokenClouds;
    } else if (description === 'overcast clouds') {
        icon.innerHTML = cloudy;
    } else if (description.includes('rain') || description.includes('drizzle')) {
        icon.innerHTML = rain;
    } else if (description.includes('thunderstorm')) {
        icon.innerHTML = storm;
    } else if (description.includes('sleet')) {
        icon.innerHTML = sleet;
...
function dayOrNight(timezone) {
    let oclock = (new Date()).getHours() + (timezone / 3600);
    console.log(oclock)
    if (oclock > 5 && oclock < 19) {
        timeOfDay = 'day';
    } else {
        timeOfDay = 'night';
    }
